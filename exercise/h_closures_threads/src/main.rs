...
fn main() {
    ...
    // the child threads.
    {
        let (tx, rx) = channel::unbounded();
        let rx2 = rx.clone();
        let channel_1 = thread::spawn(move || {
            for msg in &rx {
                println!("Received {} on ch1", msg);
            }
        });
        let channel_2 = thread::spawn(move || {
            for msg in &rx2 {
                println!("Received {} on ch2", msg);
            }
        });

        for msg in vec!["a","b","c","d","e","f"] {
            println!("Sending {} from main", msg);
            tx.send(msg).unwrap();
        }

        drop(tx);

        channel_1.join().unwrap();
        channel_2.join().unwrap();
